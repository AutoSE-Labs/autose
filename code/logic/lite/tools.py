...
TOOLS: dict[str, Tool] = {
    ...
    for t in [
        ...
        Tool(
            name="run_command",
            description=(
                "Run a shell command to gather information that cannot be obtained via file tools "
                "(e.g. git log, git status, git diff, installed package versions, linter output). "
                "Prefer file tools for reading files. Requires user approval before execution. "
                "Use only for read-only, non-destructive commands."
            ),
            parameters={
                "command": {
                    "type": "string",
...
# OpenAI function-calling schemas, generated from the TOOLS registry.
